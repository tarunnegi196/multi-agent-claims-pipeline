...
async def intake_node(state: GraphState) -> dict:
    ...
    if member is None:
        events.append(_emit(claim_id, "intake.member", TraceStatus.FAIL,
    ...
    events.append(_emit(claim_id, "intake.member", TraceStatus.PASS,
    ...
    if claim.claimed_amount < min_amt:
        events.append(_emit(claim_id, "intake.min_amount", TraceStatus.FAIL,
    ...
    events.append(_emit(claim_id, "intake.min_amount", TraceStatus.PASS,
                        detail=f"₹{claim.claimed_amount:.0f} ≥ minimum ₹{min_amt:.0f}",
                        rule="submission_rules.minimum_claim_amount", confidence=1.0))

    # 3. Documents attached
    if not claim.documents:
        events.append(_emit(claim_id, "intake.docs_present", TraceStatus.FAIL,
    ...
    events.append(_emit(claim_id, "intake.docs_present", TraceStatus.PASS,
